 ...
 *
 *   - certpulse_http_requests_total{method,path,status}
 *   - certpulse_rate_limit_hits_total{path}
 *   - certpulse_audit_log_writes_total{action,resource_type}
...
describe("Prometheus metrics — v0.4 additions", () => {
  ...
  it("exposes certpulse_http_request_duration_seconds_bucket (regression for v0.3)", async () => {
    httpRequestDurationSeconds.observe({ result: "ok", method: "GET" }, 0.05);
    const text = await metricsText();
    expect(text).toMatch(
      /certpulse_http_request_duration_seconds_bucket\{[^}]*method="GET"[^}]*\}/,
    );
  });

  it("exposes certpulse_rate_limit_hits_total after the limiter rejects", async () => {
    const app = new Hono();
    ...
    const text = await metricsText();
    expect(text).toMatch(
      /certpulse_rate_limit_hits_total\{[^}]*path="[^"]*"[^}]*\} 1/,
    );
  });

  it("exposes certpulse_audit_log_writes_total after a recordAudit call", async () => {
    recordAudit(db, {
      actorType: "system",
      actorId: null,
    ...
    const text = await metricsText();
    expect(text).toMatch(
      /certpulse_audit_log_writes_total\{[^}]*action="domain"[^}]*resource_type="domain"[^}]*\} 1/,
    );
  });

  it("exposes certpulse_last_check_timestamp_seconds (zero on a fresh DB)", async () => {
    ...
    const text = await metricsText();
    expect(text).toMatch(/certpulse_last_check_timestamp_seconds 0\b/);
  });

  it("exposes certpulse_last_alert_timestamp_seconds (zero on a fresh DB)", async () => {
    lastAlertTimestampSeconds.set(0);
    const text = await metricsText();
    expect(text).toMatch(/certpulse_last_alert_timestamp_seconds 0\b/);
  });
